...
final class MarvelRepositoryTests: XCTestCase {
    ...
    // What
    var mockNetwork: NetworkMarvelMock!
    var repository: MarvelRepository!

    // Where
    override func setUp() {
        super.setUp()
        mockNetwork = NetworkMarvelMock()
        repository = MarvelRepository(networkMarvel: mockNetwork)
    }
    ...
    func testGetCharactersReturnsExpectedResults() async {
        let characters = await repository.getCharacters()
    ...
    func testGetFullSeriesForSpiderMan() async {
        let series = await repository.getFullSeries(for: 1)
    ...
    func testGetFullSeriesForIronMan() async {
        let series = await repository.getFullSeries(for: 2)
    ...
    func testGetFullSeriesForUnknownCharacterReturnsEmpty() async {
        let series = await repository.getFullSeries(for: 999)
